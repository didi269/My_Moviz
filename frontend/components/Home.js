...
function Home() {
  ...
  useEffect(() => {
    fetch('https://my-moviz-back-psi.vercel.app/movies')
      .then(response => response.json())
      .then(data => {

        setFilmData(data.movies)
  ...
  const movies = filmData.map((data, i) => {
    const isLiked = likedMovies.some(movie => movie === data.title);
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview.length < 250 ? data.overview : data.overview.substr(1, 249) + "..."} poster={"https://image.tmdb.org/t/p/w500/" + data.poster_path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
